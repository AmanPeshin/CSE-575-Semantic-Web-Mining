...
def load_clustering(record):
    return records[record.split("'")[1]]['clustering']

def load_embedding(record):
    return records[record.split("'")[1]]['node2vec']
...
query = """
MATCH (p:Placeholder)
RETURN p.id AS id, p.degree AS degree, p.pagerank as pagerank, p.community AS community, p.localClusteringCoefficient AS clustering, p.node2vec_embedding AS node2vec
"""
...
for record in data:
    records[record['id']] = {'degree': record['degree'], 'pagerank': record['pagerank'], 'community': record['community'], 'clustering': record['clustering'], 'node2vec': record['node2vec']}
...
banksim_df['merchant_clustering'] = banksim_df['merchant'].apply(load_clustering)
banksim_df['customer_clustering'] = banksim_df['customer'].apply(load_clustering)
banksim_df['merchant_node2vec'] = banksim_df['merchant'].apply(load_embedding)
banksim_df['customer_node2vec'] = banksim_df['customer'].apply(load_embedding)

labels = banksim_df['fraud']
ori = banksim_df.copy()
# Dropping the unnecessary columns including the age and gender attributes
feature_df = banksim_df.drop(['step', 'age', 'gender', 'customer', 'zipcodeOri', 'zipMerchant', 'fraud', 'merchant_node2vec', 'customer_node2vec'], axis=1)
print(feature_df)
...
split_features1 = pd.DataFrame(ori['merchant_node2vec'].to_list(), columns=['nvm1', 'nvm2', 'nvm3', 'nvm4', 'nvm5'])
split_features2 = pd.DataFrame(ori['customer_node2vec'].to_list(), columns=['nvc1', 'nvc2', 'nvc3', 'nvc4', 'nvc5'])

scaled_df = pd.concat([scaled_df, split_features1, split_features2], axis=1)
print(scaled_df)
# scaled_df = scaled_df.drop('merchant_node2vec', axis=1)
# scaled_df = scaled_df.drop('customer_node2vec', axis=1)
...
print("\n\nBuilding Gradient Boosting Classifier with k=5 folds")
for train_index, test_index in k_fold.split(scaled_df, labels):
    ...
    gbc.fit(X_after_smote, Y_after_smote)
